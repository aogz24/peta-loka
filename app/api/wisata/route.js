...
/**
 * GET /api/wisata
 * Fetch wisata data from Supabase
 *
 * Query parameters:
...
export async function GET(request) {
  try {
    ...
    const cacheKey = cacheManager.generateKey("wisata", options);
    ...
    const data = await supabaseService.fetchWisata(options);

    const response = {
      success: true,
      data,
      count: data.length,
      message: "Wisata data fetched successfully",
      cached: false,
    };
