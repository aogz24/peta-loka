...
export class KolosalAIService {
  ...
  async generateClusteringInsight(clusteringData) {
    ...
    const { produkUnggulan, wisataMikro, trainingRecommendations, summary } =
      clusteringData;
...
WISATA MIKRO PER CLUSTER:
${wisataMikro.analysis
  .map(
    (cluster, i) => `
Cluster ${i + 1}:
...
`;
  ...
  async analyzeAreaPotential(areaData) {
    ...
    const { umkmCount, wisataCount, categories, nearestTraining, location } = areaData;
...
- Pelatihan Terdekat: ${nearestTraining && nearestTraining.length > 0
      ? nearestTraining.map((t) => `${t.name} (${t.distance.toFixed(2)} km)`).join(", ")
...
2. **Potensi Wisata**: Terdapat ${summary.totalWisata} wisata mikro yang dapat menjadi daya tarik
...
- Manfaatkan wisata mikro untuk meningkatkan traffic pelanggan
...
2. Kolaborasi dengan ${nearbyWisata.length > 0 ? nearbyWisata[0].name : 'wisata terdekat'}
...
*Catatan: Ini adalah rekomendasi demo. Untuk analisis personal, silakan konfigurasi Kolosal AI API key.*
  ...
  generateDummyAreaPotential(areaData) {
    const { umkmCount, wisataCount, categories, nearestTraining, location } = areaData;
    const rating = Math.min(10, Math.round((umkmCount + wisataCount) / 2));
    
    return `
## Analisis Potensi Area (Mode Demo)

${location ? `### Lokasi Analisis
