...
export class UserBehaviorTracker {
  ...
  getUserPreferences() {
    ...
    return {
      ...
      centerOfInterest:
        locations.length > 0 ? { lat: centerLat, lng: centerLng } : null,
...
export function generatePersonalizedRecommendations(
  ...
  if (preferences.centerOfInterest) {
    ...
    nearbyItems.forEach((item) => {
      recommendations.push({
        ...item,
        reason: `Dekat dengan area yang Anda sering lihat (${
  ...
  if (preferences.mostViewedItems.length > 0) {
    ...
    if (viewedItem) {
      const similarItems = umkmData
        ...
        .filter(
          (umkm) => !isAlreadyViewed(umkm.id, preferences.mostViewedItems)
        )
        .map((umkm) => ({
          ...umkm,
          distance: calculateDistance(
...
export function getRelatedRecommendations(
  ...
  const related = umkmData
    ...
    .map((umkm) => ({
      ...umkm,
      score:
        (umkm.categoryMatch ? 50 : 0) + Math.max(0, 50 - umkm.distance * 10),
    }))
    .sort((a, b) => b.score - a.score)
    .slice(0, maxRecommendations);
