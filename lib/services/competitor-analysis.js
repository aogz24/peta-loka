...
export function analyzeCompetitors(targetLocation, umkmData, options = {}) {
  ...
  const competitorsInRadius = umkmData
    ...
    .filter((umkm) => {
      // If includeAllCategories is true, include all
      if (includeAllCategories) return true;

      // If no categories specified, include all
      if (categoryList.length === 0) return true;

      // Otherwise, check if umkm category is in the list
      return categoryList.includes(umkm.category);
    })
  ...
  const densityZones = {
    ...
    close: competitorsInRadius.filter(
      (c) => c.distance > 0.3 && c.distance <= 0.7
    ).length,
  ...
  const proximityFactor =
    (densityZones.veryClose * 3 +
      densityZones.close * 2 +
      densityZones.moderate * 1) /
    Math.max(1, totalCompetitors);

  const intensityScore = Math.min(
    100,
    totalCompetitors * 5 + proximityFactor * 20
  );
  ...
  return {
    ...
    byCategory: Object.entries(byCategory)
      .map(([category, competitors]) => ({
        ...
        nearestDistance: competitors[0]?.distance
          ? Math.round(competitors[0].distance * 1000) / 1000
          : 0,
      }))
...
function generateCompetitorRecommendations(
  ...
  if (densityZones.veryClose > 3) {
    ...
    recommendations.push({
      ...
      description:
        "Tidak ada kompetitor sangat dekat, tapi ada di area sekitar. Posisi strategis untuk menarik customer.",
    });
  }
  ...
  if (totalCompetitors === 0) {
    recommendations.push({
      ...
      description:
        "Tidak ada kompetitor mungkin berarti tidak ada demand. Lakukan riset pasar mendalam sebelum membuka usaha.",
    });
    ...
    recommendations.push({
      ...
      description:
        "Jumlah kompetitor ideal. Ada demand yang terbukti tanpa saturasi berlebihan.",
    });
  }
...
export function compareLocations(location1, location2, umkmData, options = {}) {
  ...
  const comparison = {
    ...
    comparison: {
      competitors: {
        ...
        winner:
          analysis1.summary.totalCompetitors <
          analysis2.summary.totalCompetitors
            ? "location1"
            : "location2",
      },
      opportunity: {
        ...
        winner:
          analysis1.summary.opportunityScore >
          analysis2.summary.opportunityScore
            ? "location1"
            : "location2",
      },
      intensity: {
        ...
        winner:
          analysis1.summary.intensityScore < analysis2.summary.intensityScore
            ? "location1"
            : "location2",
      },
    },
  ...
  comparison.winner =
    wins.location1 > wins.location2 ? "location1" : "location2";
...
export function findMarketGaps(
  ...
  const existingCategories = existingAnalysis.byCategory.map(
    (cat) => cat.category
  );
