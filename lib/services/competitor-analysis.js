...
export function analyzeCompetitors(targetLocation, umkmData, options = {}) {
  ...
  return {
    ...
    densityZones,
    byCategory: Object.entries(byCategory).map(([category, competitors]) => ({
      category,
...
export function compareLocations(location1, location2, umkmData, options = {}) {
  ...
  comparison.recommendation = wins.location1 > wins.location2
...
export function findMarketGaps(targetLocation, umkmData, allCategories = [], options = {}) {
  ...
  const existingAnalysis = analyzeCompetitors(
    targetLocation,
    umkmData,
    { ...options, includeAllCategories: true }
  );
  ...
  // Find gaps
  const gaps = allCategories.map(category => {
