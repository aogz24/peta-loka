...
export class ClusteringService {
  /**
   * Menghitung Silhouette Coefficient untuk evaluasi kualitas clustering
  ...
  calculateSilhouetteCoefficient(points, clusters) {
    ...
    // Jika hanya ada 1 cluster, silhouette score = 0
    ...
    for (let i = 0; i < n; i++) {
      ...
      // a(i): rata-rata jarak ke semua point dalam cluster yang sama
      const sameClusterPoints = points.filter((_, idx) => clusters[idx] === currentCluster);
      let a = 0;
      if (sameClusterPoints.length > 1) {
        for (let j = 0; j < n; j++) {
          if (i !== j && clusters[j] === currentCluster) {
            a += this.calculateDistance(points[i][0], points[i][1], points[j][0], points[j][1]);
          }
        }
        a /= (sameClusterPoints.length - 1);
      }

      // b(i): rata-rata jarak minimum ke point di cluster lain
      let b = Infinity;
      for (const otherCluster of uniqueClusters) {
        if (otherCluster === currentCluster) continue;
        
        const otherClusterPoints = points.filter((_, idx) => clusters[idx] === otherCluster);
        if (otherClusterPoints.length === 0) continue;

        let avgDist = 0;
        for (let j = 0; j < n; j++) {
          if (clusters[j] === otherCluster) {
            avgDist += this.calculateDistance(points[i][0], points[i][1], points[j][0], points[j][1]);
          }
        }
        avgDist /= otherClusterPoints.length;
  ...
  /**
   * Menemukan jumlah cluster optimal menggunakan Silhouette Coefficient
  ...
  findOptimalClusters(data, minClusters = 2, maxClusters = 10) {
    ...
    if (validData.length < minClusters) {
      return { optimalK: Math.max(2, validData.length), scores: [], bestScore: 0 };
    }
    ...
    for (let k = minClusters; k <= Math.min(maxClusters, validData.length); k++) {
      const result = kmeans(points, k, {
        initialization: "kmeans++",
        maxIterations: 100,
      });

      const silhouetteScore = this.calculateSilhouetteCoefficient(points, result.clusters);
  ...
  clusterByLocation(data, numClusters = null) {
    ...
    const result = kmeans(points, finalNumClusters, {
      initialization: "kmeans++",
      maxIterations: 100,
    });
