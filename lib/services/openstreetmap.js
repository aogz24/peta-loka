...
export class OpenStreetMapService {
  constructor() {
    this.overpassUrl = "https://overpass.kumi.systems/api/interpreter";
  ...
  async scrapePelatihan(lat, lon, radius = 5000) {
    const query = `
      ...
      (
        ...
        node["amenity"="vocational_school"](around:${radius},${lat},${lon});
        ...
        way["amenity"="vocational_school"](around:${radius},${lat},${lon});
      );
      out center;
    `;
   ...
   */
  async scrapeAll(lat, lon, radius = 10000) {
    try {
  ...
  /* ============================================================
     TAMBAHAN: FUNGSI UNTUK MENYIMPAN HASIL SCRAPING KE FILE
     ============================================================ */
  ...
  /**
   * Scrape semua → lalu simpan ke file JSON di /lib/data/
   */
  async scrapeAndSave(lat, lon, radius = 10000) {
    console.log("Scraping data dari OSM...");
    const data = await this.scrapeAll(lat, lon, radius);

    this.saveJSON("umkm.json", data.umkm);
    this.saveJSON("wisata.json", data.wisata);
    this.saveJSON("pelatihan.json", data.pelatihan);
