 ...
 */
export function calculateLocationPotential(
  location,
  umkmData,
  wisataData,
  pelatihanData,
  ...
  const nearestWisata = wisataDistances
    .sort((a, b) => a.distance - b.distance)
    .slice(0, 5);
  const avgWisataDistance =
    nearestWisata.length > 0
      ? nearestWisata.reduce((sum, w) => sum + w.distance, 0) /
  ...
  const nearestPelatihan = pelatihanDistances
    .sort((a, b) => a.distance - b.distance)
    .slice(0, 3);
  const avgPelatihanDistance =
    nearestPelatihan.length > 0
      ? nearestPelatihan.reduce((sum, p) => sum + p.distance, 0) /
        nearestPelatihan.length
      : searchRadius * 2;
    ...
    details: {
      competitors: {
        ...
        list: competitors.slice(0, 10).map((c) => ({
          ...
          distance:
            Math.round(
              calculateDistance(location.lat, location.lng, c.lat, c.lon) * 1000
            ) / 1000,
        })),
      },
      wisata: {
 ...
 */
export function findPotentialLocations(
  umkmData,
  wisataData,
  pelatihanData,
  options = {}
 ...
 */
export function analyzeSpecificLocation(
  lat,
  lng,
  umkmData,
  wisataData,
  ...
  return calculateLocationPotential(
    location,
    umkmData,
    wisataData,
    pelatihanData,
