  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* UMKM per Cluster */}
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-lg font-bold text-gray-800 mb-4">UMKM per Cluster</h3>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total UMKM</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kategori Dominan</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {produkUnggulan.analysis.map((cluster, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Cluster {index + 1}</td>
      ...
      {/* Wisata Potensi */}
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h3 className="text-lg font-bold text-gray-800 mb-4">Potensi Wisata Mikro</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {wisataMikro.analysis.map((cluster, index) => (
            <div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              ...
              <p className="text-sm text-gray-600 mt-1">Total Wisata: {cluster.totalWisata}</p>
              <p className="text-sm mt-1">
                Potensi:
                <span className={`ml-2 font-semibold ${cluster.potensi === 'Sangat Tinggi' ? 'text-green-600' : cluster.potensi === 'Tinggi' ? 'text-blue-600' : cluster.potensi === 'Sedang' ? 'text-amber-600' : 'text-gray-600'}`}>
                  {cluster.potensi}
                </span>
              </p>
            </div>
