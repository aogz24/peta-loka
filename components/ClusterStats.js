        ...
        <div className="glass-card flex flex-col p-4">
          ...
          {summary.method && (
            <div className="mt-1 text-xs text-zinc-400 dark:text-zinc-500">
              {summary.method === "silhouette-optimized"
                ? "🎯 Auto-optimized"
                : "✋ Manual"}
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                Kualitas Clustering (Silhouette Score)
              </div>
              <div className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                Skor -1 hingga 1, semakin tinggi semakin baik kualitas
                clusternya
              </div>
            </div>
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-300">
                    <ClusterLocationButton
                      ...
                      onClick={() =>
                        onClusterSelect &&
                        onClusterSelect(cluster.center.lat, cluster.center.lon)
                      }
      ...
      <div className="glass-card p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {wisata.analysis.map((cluster, index) => (
            <div
              key={index}
              className="p-4 rounded-lg border border-zinc-100 dark:border-zinc-800 bg-white/60 dark:bg-zinc-900/60"
            >
              <div className="flex items-center justify-between">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-300 mt-2">
                Total Wisata:{" "}
                <span className="font-medium">{cluster.totalWisata}</span>
              </p>
            </div>
          ))}
        </div>
