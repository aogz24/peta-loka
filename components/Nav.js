...
export default function Nav() {
  ...
  ${isActive ? 'text-blue-600 dark:text-blue-300 bg-blue-500/10 dark:bg-blue-400/10 shadow-sm shadow-blue-500/20' : 'text-zinc-700 dark:text-zinc-200 hover:text-blue-600 dark:hover:text-blue-300'}
`}
                >
                  {item.label}
                  <span
    ...
    ${isActive ? 'w-3/4 -translate-x-1/2 opacity-100' : 'w-0 opacity-0 group-hover:w-1/2 group-hover:opacity-100 group-hover:-translate-x-1/2'}
  `}
                  />
                </Link>
              );
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Mobile Toggle */}
            <button onClick={() => setOpen((s) => !s)} aria-label="Toggle menu" className="md:hidden p-2 rounded-lg hover:bg-zinc-200/60 dark:hover:bg-zinc-800/60 dark:text-zinc-200 transition">
        ...
        <div className="md:hidden bg-white/90 dark:bg-zinc-900/90 backdrop-blur-lg border-t border-white/20 dark:border-white/5 animate-fadeIn">
          <div className="px-4 pt-4 pb-6 space-y-4">
            {[
              ...
              const isActive = pathname === item.href || (item.href !== '/' && pathname?.startsWith(item.href + '/')) || pathname === item.href + '/';
              return (
                <Link key={item.href} href={item.href} className={`block text-base font-medium ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-zinc-800 dark:text-zinc-200'}`} onClick={() => setOpen(false)}>
                  {item.label}
                </Link>
              );
            })}
