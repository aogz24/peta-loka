...
export default function LocationPredictionPanel({ onLocationSelect }) {
  ...
  const handleScanArea = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/location-prediction?mode=scan&topN=10&minScore=50');
  ...
  const handleAnalyzeLocation = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/location-prediction?mode=analyze&lat=${selectedLat}&lng=${selectedLng}&searchRadius=${searchRadius}`);
      const data = await response.json();
    ...
    <div className="space-y-4">
      ...
      {mode === 'scan' && (
        <div className="space-y-3">
          <p className="text-sm text-gray-600">Scan seluruh area untuk menemukan lokasi potensial dengan skor tertinggi</p>
          <button
      ...
      {mode === 'analyze' && (
        <div className="space-y-3">
          <p className="text-sm text-gray-600 dark:text-gray-200">Klik pada peta atau masukkan koordinat untuk analisis lokasi spesifik</p>
      ...
      {results && (
        <div className="mt-6 space-y-4">
          {results.mode === 'scan' && (
            <>
              <h3 className="font-semibold text-lg">Top {results.count} Lokasi Potensial</h3>
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {results.locations.map((loc, idx) => (
                  <div key={idx} className={`p-4 rounded-lg border-2 cursor-pointer hover:shadow-md transition-shadow ${getRatingColor(loc.rating)}`} onClick={() => onLocationSelect && onLocationSelect(loc.location.lat, loc.location.lng)}>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="px-3 py-1 rounded-full text-xs font-bold">{loc.rating}</div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 text-xs">
                      ...
                      <div>
                        ...
                        <div>{loc.details.pelatihan.nearest.length} terdekat</div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {results.mode === 'analyze' && results.result && (
            <>
              ...
              <div className={`p-4 rounded-lg border-2 ${getRatingColor(results.result.rating)}`}>
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <div className="text-2xl font-bold">Score: {results.result.score}</div>
                    <div className="text-sm opacity-75">
                      {results.result.location.lat.toFixed(6)}, {results.result.location.lng.toFixed(6)}
                    </div>
                  </div>
                  <div className="px-3 py-1 rounded-full text-sm font-bold">{results.result.rating}</div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="bg-white dark:bg-gray-900 bg-opacity-50 p-3 rounded">
                    ...
                    <div className="text-sm space-y-1">
                      {results.result.details.competitors.list.slice(0, 3).map((comp, i) => (
                        <div key={i} className="flex justify-between">
                          <span>{comp.name}</span>
                          <span className="text-xs opacity-75">{comp.distance} km</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-900 bg-opacity-50 p-3 rounded">
                    ...
                    <div className="text-sm space-y-1">
                      {results.result.details.wisata.nearest.map((wisata, i) => (
                        <div key={i} className="flex justify-between">
                          <span>{wisata.name}</span>
                          <span className="text-xs opacity-75">{wisata.distance} km</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-900 bg-opacity-50 p-3 rounded">
                    ...
                    <div className="text-sm space-y-1">
                      {results.result.details.pelatihan.nearest.map((pelatihan, i) => (
                        <div key={i} className="flex justify-between">
                          <span>{pelatihan.name}</span>
                          <span className="text-xs opacity-75">{pelatihan.distance} km</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="h-[calc(100%-60px)]">
              <MapComponent
                ...
                onSelectLocation={(selectedLat, selectedLng) => {
                  setSelectedLat(selectedLat.toFixed(6));
                  setSelectedLng(selectedLng.toFixed(6));
