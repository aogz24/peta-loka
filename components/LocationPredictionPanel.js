  ...
  return (
    <div className="space-y-4">
      ...
      {mode === 'scan' && (
        <div className="space-y-3">
          <p className="text-sm text-gray-600">
            Scan seluruh area untuk menemukan lokasi potensial dengan skor tertinggi
          </p>
          <button
      ...
      {mode === 'analyze' && (
        <div className="space-y-3">
          <p className="text-sm text-gray-600">
            Klik pada peta atau masukkan koordinat untuk analisis lokasi spesifik
          </p>
            ...
            <>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {results.locations.map((loc, idx) => (
                  <div
                    key={idx}
                    className={`p-4 rounded-lg border-2 cursor-pointer hover:shadow-md transition-shadow ${getRatingColor(loc.rating)}`}
                    onClick={() => onLocationSelect && onLocationSelect(loc.location.lat, loc.location.lng)}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <div className="font-semibold">#{idx + 1} Score: {loc.score}</div>
